...
class DashboardBlock(models.Model):
    ...
    def get_dashboard_vals(self, action_id, start_date=None, end_date=None):
        ...
        for rec in self.env['dashboard.block'].sudo().search(
            ...
            filter_list = [f for f in filter_list if not (isinstance(f, tuple) and f[0] == 'company_id')]

            # Add allowed company_id filter
            allowed_company_ids = self.env.context.get('allowed_company_ids', [])
            if allowed_company_ids:
                filter_list.append(('company_id', 'in', allowed_company_ids))
