...
import {is_option_set} from "@access_control/components/relational_utils.esm";
...
CreateConfirmationDialog.template =
    "access_control.Many2OneField.CreateConfirmationDialog";
    ...
    async openConfirmationDialog(request) {
        var m2o_dialog_opt =
            ...
            (this.props.nodeOptions.m2o_dialog === undefined &&
                is_option_set(this.ir_options["access_control.m2o_dialog"])) ||
            (this.props.nodeOptions.m2o_dialog === undefined &&
                this.ir_options["access_control.m2o_dialog"] === undefined);
...
patch(FormController.prototype, {
    ...
    async _setSubViewLimit() {
        const ir_options = Component.env.session.access_control;

        const activeFields = this.archInfo.activeFields,
            fields = this.props.fields,
            isSmall = this.user;

        var limit = ir_options["access_control.field_limit_entries"];
        if (!(limit === undefined)) {
